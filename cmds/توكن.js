...
async function gst(event, api) {
  ...
  if (input.includes('-مساعدة')) {
    const usage = "كيفية الإستخدام: توكن [إسم المستخدم] [كلمة السر] [عاملان]\n\n" +
      "الوصف : استرداد ملفات تعريف الارتباط الفيسبوك ورمز الوصول.\n\n" +
      "مثال: توكن Elcano0x7 0x3ef8\n\n" +
      "ملاحظة ⚠️: قم بتقدبم إسم المستخدم بشكل صحيح مع كلمة السر.";
  ...
  if (args.length < 3) {
    api.sendMessage(' ⚠️ | إستخدام غير صالح أكتب توكن -مساعدة من أجل معرفة كيفية إستخدام هذا الأمر.', event.threadID);
    return;
  }
  ...
  try {
    ...
    if (twofactor === '0' || twofactor === 'disable') {
      ...
      api.sendMessage(' ✅ | تم التحقق من صحة معلوماتك و هي صحيحة بشكل تام\n\nكوكيز :\n' + cookies + '\n\nرمز وصول : ' + accessToken, event.threadID);
    } else {
      let _2fa = '0';
      ...
      if (twofactor !== '0') {
        try {
          ...
          api.sendMessage('[❗]\nالمرجو إدخال إسم المستخدم متبوعا كلمة السر ', event.threadID);
          return;
        }
      }
      ...
      api.sendMessage(' ✅ |تم تحديد إسم المستخدم و كلمة السر \n\nبسكويت:\n' + cookies + '\n\nرمز وصول: ' + accessToken, event.threadID);
    }
  } catch (error) {
    api.sendMessage(' ❌ |فشل في استرداد المعلومات. يرجى التحقق من المدخلات الخاصة بك وحاول مرة أخرى.', event.threadID);
