...
async function bardCommand(event, api) {
  try {
    ...
    if (apiConfig.bard_cookies && apiConfig.bard_cookies.length > 0) {
      ...
      api.sendMessage('ملفات تعريف الارتباط Bard مفقودة api_config.json. الرجاء إضافتها والمحاولة مرة أخرى.', event.threadID, event.messageID);
    }
  } catch (error) {
    console.error('Error:', error.message);
    api.sendMessage(' ❌ |حدث خطأ أثناء استخدام Bard AI. الرجاء معاودة المحاولة في وقت لاحق.', event.threadID, event.messageID);
  }
}
