...
const nero = require('./0x3/moduleInstaller.js');
...
figlet.text(
  ...
  async (err, data) => {
    if (err) {
      logError("Error rendering itashibot ASCII art.", err);
      return;
    }

    console.log(chalk.cyan(data));
    console.log(chalk.gray("modified by HUSSEIN | ©️ 2023"));
    console.log(chalk.gray("This bot is not for sale"));
    console.log(chalk.gray("Repository: https://github.com/HUSSEINHN123/ITASHI"));
    console.log();
              ...
              if (!mutedUsersReminded.has(userId)) {
                api.getUserInfo(userId, async (err, ret) => {
                  if (!err && ret && ret[userId] && ret[userId].name) {
                    ...
                    api.sendMessage(`مرحبا ! ${userName}\n ⚠️ | تم كتم صوتك حاليًا. يرجى الانتظار بصبر لحوالي ${(MUTE_DURATION - (Date.now() - timestamp)) / 1000} ثواني لاستعادة الوصول. \n\nيرجى ملاحظة أن هذه الرسالة هي إشعار لمرة واحدة ولن يتم إرسالها مرة أخرى لتجنب أي إزعاج.`, event.threadID, event.messageID);
                ...
                if (matchingCommand) {
                  ...
                  if (recentCommands.length > COMMAND_LIMIT && isNotAuthorized) {
                    if (!mutedUsers.has(userId)) {
                      ...
                      api.sendMessage(`أوه لا، أنت تسير بسرعة كبيرة. لقد تم كتم صوتك ل ${MUTE_DURATION / 1000} ثواني للاستخدام المفرط للأوامر.`, event.threadID, event.messageID);
                    }
                  } else {
